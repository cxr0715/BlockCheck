...
+ (NSMutableSet *)getBlockStrongLayout_new:(void *)block//通过block_extended_layout获取强引用信息
{
    ...
    if (firstlayout < 0x1000) { //0xXYZ模式
        ...
        for (int i = 0; i < bbyref; ++i) {
            ...
            void *ss = NULL;
            struct Block_byref_block *byrefBlock = (struct Block_byref_block *)s;
            if ( (byrefBlock->flags & BLOCK_BYREF_LAYOUT_STRONG) == BLOCK_BYREF_LAYOUT_STRONG ) {
                ss = byrefBlock->capture;
            }
            if ( ss != NULL ) {
